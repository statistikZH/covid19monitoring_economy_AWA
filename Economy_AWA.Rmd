...
krzarbeit <- read_xlsx('KAE Voranm KTZH.xlsx',sheet ='KArb VA U AN täglich Kanton ...') %>%
  ...
  mutate(variable_long = ifelse(variable_short=='kum_kurzarbeitzh_bew_betrieb','Kummulierte bewilligte Voranmeldungen Kurzarbeit Betriebe','Kummulierte bewilligte Voranmeldungen Kurzarbeit voraussichtlich betroffene Arbeitn.'),
...
#Datensatz speichern
write.csv(krzarbeit,'Economy_AWA_entwurf.csv',row.names=F, fileEncoding = "UTF-8")
View(krzarbeit)
...
krzarbeit_bis5April <- read_xlsx('KAE Voranm KTZH.xlsx',sheet ='KArb VA U AN täglich Kanton ...') %>%
  ...
  #tidyr::pivot_longer(-date,names_to='variable_short',values_to='value')%>%
  tidyr::gather('variable_short','value',-date)%>%
  mutate(variable_long = ifelse(variable_short=='kum_kurzarbeitzh_bew_betrieb_bis_5april','Kummuliertes Total Bewilligte Voranmeldungen Kurzarbeit Betriebe bis 5 April 2020','Kummuliertes Total Bewilligte Voranmeldungen Kurzarbeit voraussichtlich betroffene Arbeitn. bis 5. April 2020'),
         description = 'https://github.com/statistikZH/covid19monitoring_economy_AWA',
         location = 'ZH',
         topic = 'Wirtschaft',
         unit = 'Anzahl',
...
write.csv(krzarbeit_bis5April,'Economy_AWA_total_bis_5april_entwurf.csv',row.names=F, fileEncoding = "UTF-8")
```