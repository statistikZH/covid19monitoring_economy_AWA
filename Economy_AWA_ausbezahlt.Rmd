...
krzarb_bezahlt <- krzarbeit_bezahlt %>%
  ...
  filter(!is.na(kurzarbeitzh_ausbezahlt_betrieb) | !is.na(kurzarbeitzh_ausbezahlt_arbeitn))%>%
...
krzarb_bezahlt2 <- krzarb_bezahlt%>%
  ...
  select(-monat,-jahr)%>%
  mutate(share_ausbezahlt_besch  = kurzarbeitzh_ausbezahlt_arbeitn/angestellte,
         share_ausbezahlt_betriebe = kurzarbeitzh_ausbezahlt_betrieb/betriebe) %>%
  ...
  mutate(variable_long = case_when(
    variable_short=='kurzarbeitzh_ausbezahlt_betrieb' ~ 'Kurzarbeit ausbezahlt Anzahl Betriebe',
    variable_short=='kurzarbeitzh_ausbezahlt_arbeitn' ~ 'Kurzarbeit ausbezahlt Anzahl Beschäftigte',
    variable_short=='share_ausbezahlt_besch' ~ 'Anteil Beschäftigte',
    variable_short=='share_ausbezahlt_betriebe' ~ 'Anteil Betriebe',
     TRUE ~ 'NA'),

    variable_short = ifelse(variable_short == 'kurzarbeitzh_ausbezahlt_arbeitn', 'kurzarbeitzh_ausbezahlt_besch', variable_short),
...
datalist[[counter]] <- krzarb_bezahlt2
...
pldf <-  Economy_AWA_ausbezahlt_plotstart%>%
  ...
  mutate(share_ausbezahlt_arbeitn = kurzarbeitzh_ausbezahlt_arbeitn/anz_angestellte,
         share__ausbezahlt_betriebe = kurzarbeitzh_ausbezahlt_betrieb/anz_betriebe)%>%
  ...
  mutate(label = paste0(round(value*100,2),' %'))%>%
  mutate(variable_short = ifelse(variable_short=='share_ausbezahlt_arbeitn','Anteil Beschäftigte','Anteil Betriebe'),
...
ggsave('plots/Economy_AWA_ausbezahlt_branchen_short_two_months2.png',width = 22.86, height =14.128, units = c("cm"),dpi=300,scale=3)

```

```{r tweet plot}
ggplot(pldf[pldf$month=='April',],aes(noga_string,value,fill=variable_short))+
  geom_col(position = 'dodge')+
  geom_text(aes(label = label, y = (value+0.01)),position = position_dodge(width = 1),size=8, hjust=0)+
  scale_y_continuous(labels = scales::percent, limits=c(0,1))+
  coord_flip()+
  ...
  scale_fill_manual(values=zhpal$zh[c(1,3)], breaks= c('Anteil Betriebe','Anteil Beschäftigte'))+
    theme(legend.position = "top",
        legend.title=element_blank()) +
  theme(
    axis.line = element_line(color="black", size = 1),
...
ggsave('plots/Economy_AWA_ausbezahlt_branchen_tweet2.png',width = 22.86, height =14.128, units = c("cm"),dpi=300,scale=3)
```
